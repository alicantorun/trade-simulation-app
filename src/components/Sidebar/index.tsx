...
const Sidebar: React.FC<ISidebar> = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  useEffect(() => {
    const clickHandler = ({ target }: MouseEvent) => {
      ...
      if (
        !isSidebarOpen ||
        sidebar.current.contains(target as any) ||
        trigger.current.contains(target as any)
      )
        return;
  ...
  return (
    <div className="lg:w-64">
      ...
      <div
        id="sidebar"
        ref={sidebar}
        className={`absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 transform h-screen overflow-y-auto lg:overflow-y-auto no-scrollbar w-64 flex-shrink-0 bg-gray-800 p-4 transition-transform duration-200 ease-in-out ${
